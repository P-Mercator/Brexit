...
* DONE DescriptiNOTE: many reconciliation services are often disrupted. Some Open Refine distributions have implemented these services,  but not all of them work as it is supposed do do. An alternative way is merging columns from different datasets with GREL. A step-by-step tutorial can be found here (previously you have to download the dataset, for instance by querying DBpedia SPARQL)on of variables and recoding (2 page max., 0.5 pt) [3/3]
...
#+BEGIN_SRC R :exports results :results graphics :file "./pictures/regionLeave.svg"
  ggplot(na.omit(brexit),aes(Region,Pct_Leave))+
    geom_boxplot()+
    geom_hline(linetype = 2,yintercept = 50,color="red")+
    theme(axis.text.x = element_text(angle = 90, hjust = 1))
#+END_SRC
...
#+RESULTS:
[[file:./pictures/regionLeave.svg]]

#+BEGIN_SRC R :exports results :results graphics :file "./pictures/QualQuanMatrics.svg"
...
#+BEGIN_SRC R :exports results :results graphics :file "./pictures/QuanMatrics.svg"
...
#+BEGIN_SRC R :exports results :results graphics :file "./pictures/ageLeave.svg"
  ggplot(na.omit(brexit),aes(Mean.Age,Pct_Leave))+
    geom_point()+
    geom_smooth(method = "loess", se = TRUE)
#+END_SRC

#+RESULTS:
[[file:./pictures/ageLeave.svg]]
